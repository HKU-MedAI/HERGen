...
class ReportLogger(Metric):
    ...
    def update(self, reports, dicom_ids=None):
        ...
        assert isinstance(
            reports, list), '"reports" must be a list of strings.'
        assert all(isinstance(i, str)
                   for i in reports), 'Each element of "reports" must be a string.'
    ...
    def log(self, epoch):
        ...
        def save():
            ...
            df.to_csv(
                os.path.join(
                    self.save_dir, f'{self.split}_epoch-{epoch}_{time.strftime("%d-%m-%Y_%H-%M-%S")}.csv'),
                index=False,
            )
